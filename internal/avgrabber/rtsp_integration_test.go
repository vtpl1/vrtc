...
// Integration tests for the avgrabber package against a live RTSP camera.
// The camera at testRTSPURL must be reachable on the network.
...
func rtspConfig() avgrabber.Config {
	return avgrabber.Config{
		URL:      testRTSPURL,
		Username: testRTSPUsername,
		Password: testRTSPPassword,
...
// TestSession_RTSP_Open verifies that the C library can open and close the
...
func TestSession_RTSP_Open(t *testing.T) {
	...
	s, err := avgrabber.Open(rtspConfig())
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
...
func TestSession_RTSP_StreamInfo(t *testing.T) {
	...
	// Poll until StreamInfo is ready (first PARAM_SET received).
	deadline := time.Now().Add(streamTimeout)
...
func TestSession_RTSP_Stats(t *testing.T) {
	...
	// Drain at least minVideoFrames of video.
	deadline := time.Now().Add(streamTimeout)
	videoFrames := 0
...
func TestDemuxer_RTSP_GetCodecs(t *testing.T) {
	...
	dmx, err := avgrabber.NewDemuxer(rtspConfig())
	if err != nil {
...
func TestDemuxer_RTSP_StopResume(t *testing.T) {
	...
	// Read a few packets to confirm the stream is live.
	for range 10 {
	...
	// Stream must recover and deliver new packets.
	videoAfterResume := 0
