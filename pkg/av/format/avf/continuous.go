...
// ErrNoAVFFiles is returned by NewContinuous when no *.avf files are found.
var ErrNoAVFFiles = errors.New("avf: no *.avf files found")
...
func NewContinuous(baseDir string, opts ...Option) (*ContinuousDemuxer, error) {
	...
	if len(files) == 0 {
		return nil, fmt.Errorf("%w in %q", ErrNoAVFFiles, baseDir)
	}
...
func fileDateTime(path string) time.Time {
	...
	t, err := time.ParseInLocation("20060102_150405", m, time.UTC)
	if err != nil {
		return time.Time{}
	}
