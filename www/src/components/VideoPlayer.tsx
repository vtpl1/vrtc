...
const VideoPlayer = () => {
  ...
  const [url, setUrl] = useState<string | null>("ws://localhost:1984/api/ws?src=1");
  ...
  const handleClickSendMessage = useCallback(() => sendMessage(JSON.stringify({ "type": "mse" })), []);
  ...
  useEffect(() => {
    ...
    if (typeof ev.data === "string") {
      ...
      if (msg.type === "mse") {
        ...
        // if (lastJsonMessage === "mse") {
        //   console.log(lastJsonMessage, "mse")
        // } else {
  ...
  //   if (lastJsonMessage["type"] === "mse") {
  //     mseCodecs = lastJsonMessage["value"];
  //   }
  //   if (msRef.current === null)
  //     return
  ...
  useEffect(() => {
    ...
    if (videoRef?.current) {
      start(videoRef.current, msRef.current, debouncedSiteId, debouncedAppId, debouncedChannelId, debouncedStreamId, debouncedLiveOrRecording, debouncedTimeStamp)
    }
    return () => {
